...
def min_len_for_task(task_name, default_min, min_summary, min_qa, min_keypoints, min_title):
    ...
    if "qa" in t:
        return min_qa
    if "key_points" in t or "keypoints" in t or "key-points" in t:
        return min_keypoints
    if "title" in t:
        return min_title
    return default_min
...
def score_row(row, rep_thresh, min_summary, min_qa, min_keypoints, min_title):
    ...
    needed = min_len_for_task(
        task, default_min=10, min_summary=min_summary, min_qa=min_qa, min_keypoints=min_keypoints, min_title=min_title
    )
    if len(output.strip()) < needed:
        flags.append("short_output")
    if has_refusal(output):
        flags.append("possible_refusal")
    if bigram_repetition_ratio(output) >= rep_thresh:
        flags.append("high_repetition")
    if question and answer and context and weak_grounding(answer, context):
        flags.append("weak_grounding")

    score = 1.0
    if "short_output" in flags:
        score -= 0.40
    if "possible_refusal" in flags:
        score -= 0.60
