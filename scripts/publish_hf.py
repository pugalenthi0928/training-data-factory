...
def main():
    ...
    try:
        ...
        print(f"[publish_hf] create_repo failed: {e}\nHint: ensure HUGGINGFACE_TOKEN has repo WRITE and the namespace matches your account.", flush=True)
        raise

    for p in args.path:
        pth = Path(p)
        if not pth.exists():
            print(f"Skip missing: {pth}", file=sys.stderr); continue
        api.upload_file(
