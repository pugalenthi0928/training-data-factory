...
def test_quality_flags_and_score(tmp_path: Path):
    ...
    rows = [
        ...
        {"task_name": "summary_v1", "output_text": "Too short"},
        {
            ...
            "answer": "N/A",
            "context": "N/A",
            "output_text": "As an AI language model, I cannot provide that.",
        },
        {"task_name": "key_points_v1", "output_text": "alpha beta alpha beta alpha beta alpha beta"},
        {
            ...
            "context": "This passage mentions famous museums in Paris.",
            "output_text": "The Eiffel Tower is in Paris.",
        },
    ]
    wjsonl(inp, rows)
    r = subprocess.run(
        [sys.executable, "scripts/quality_filters.py", "--input", str(inp), "--output", str(outp)],
        capture_output=True,
        text=True,
    )
